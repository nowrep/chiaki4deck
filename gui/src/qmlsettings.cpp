...
QString QmlSettings::changeControllerKey(int button)
{
    QString out;
    SettingsKeyCaptureDialog dialog;
    connect(&dialog, &SettingsKeyCaptureDialog::KeyCaptured, [this, button, &out](Qt::Key key) {
        out = QKeySequence(key).toString();
        settings->SetControllerButtonMapping(button, key);
    });
    dialog.exec();
    return out;
}
